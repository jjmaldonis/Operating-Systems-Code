...
PRIVATE int open_counter; //initialized in sef_cb_init

PRIVATE int counter_open(message *UNUSED(m))
{
    ++open_counter; //increment counter now
    return OK;
}
        ...
        iovec_t *iov, unsigned nr_req, endpoint_t UNUSED(user_endpt))
{
    ...
    {
        case DEV_GATHER_S:
            //this next function is where the serious stuff happens. i've detailed a bit about it below in these comments
...
PRIVATE int sef_cb_init(int type, sef_init_info_t *UNUSED(info))
{
    ...
    open_counter = -1; //initialize counter to -1 so that when we increment it upon start it starts at 0
    switch(type) {
        case SEF_INIT_FRESH:
            break;
